...
interface Slots {
  add(
    name: string,
    content: string | Message | Message[] | Input | Attachment | Attachment[] | null,
    options?: {
      priority?: number
      maxRatio?: number
      minRatio?: number
...
If a role is not specified, the runtime chooses a default role based on the slot name:

- if the slot name is `system`, the default role is `system`
- otherwise, the default role is `user`

This same defaulting rule applies when the content is a string, `Input`, or attachments.
