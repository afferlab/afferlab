...
Tool-related option fields are reserved for advanced host-managed flows.
They are not part of the supported v0.1 main conversation flow.
...
When you use the `LLMMessage[]` overload, you may also provide additional call options such as `temperature`.

## run(options)

`run()` executes a host-managed multi-round model loop.

Use it when a strategy needs a bounded multi-round model interaction.
